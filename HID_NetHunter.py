...
class HID_NetHunter:
    def __init__(self):
        ...
        self._release = b'\x00\x00\x00\x00\x00\x00\x00\x00'

    def send_text(self, str):
        for key in str:
            if key in self._keys:
                with open('/dev/hidg0', 'rb+') as fd:
                    fd.write(self._keys[key])
                    time.sleep(0.005)
                    fd.write(self._release)
    ...
    def send_combo(self, *keys):
        ...
        for key in keys:
            ...
            # Normal key
                ...
                if actual not in self._KEYS:
                    raise ValueError(f"Unknown key: {key}")

                keycodes.append(self._KEYS[actual])
